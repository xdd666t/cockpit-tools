...
### 新增
- **指纹页新增"清理未绑定"功能**：工具栏新增"清理未绑定"按钮，可一键批量删除所有未绑定账号的自定义指纹，并实时显示可清理数量。

### 变更
- **插件导入改为严格使用 Antigravity SecretStorage**：Antigravity「从插件导入」仅从 Antigravity 用户目录下的 `state.vscdb` 读取并解密凭据，不再回退到 VS Code profile 存储。
- **插件导入改为“仅新增”模式（不覆盖更新）**：导入过程中遇到本地已存在账号将直接跳过，不再原地更新；成功数仅统计新增加账号。
- **插件导入支持实时进度并在导入后立即刷新订阅**：导入阶段按账号推送 `current/total` 进度，且对新导入账号立即刷新配额/订阅数据，减少初始 `UNKNOWN` 套餐标识。
